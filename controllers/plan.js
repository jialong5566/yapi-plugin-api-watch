...
class ApiWatchPlanController extends baseController {
  constructor(ctx) {
    super(ctx);
  ...
  async saveApiWatchPlan(ctx) {
    ...
    if (!params.env_id) {
      return (ctx.body = yapi.commons.resReturn(null, 400, '测试环境不能为空'));
    }
    ...
    try {
      ...
      if (data.is_plan_open) {
        this.schedule.addApiWatchJob(plan._id, plan);
      }
  ...
  async updateApiWatchPlan(ctx) {
    ...
    try {
      ...
      //操作定时任务
      if (data.is_plan_open) {
        this.schedule.addApiWatchJob(id, plan);
      } else {
        this.schedule.deleteApiWatchJob(id);
      }
      ctx.body = yapi.commons.resReturn(plan);
    } catch (e) {
      ctx.body = yapi.commons.resReturn(null, 401, e.message);
...
module.exports = ApiWatchPlanController;